...
class myDenseNet(nn.Module):
    def __init__(
        ...
        self.classifier3 = nn.Linear(num_features, 128, bias=False)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        p, p1 = self.cal(out)
        p = self.classifier3(p)
        p1 = self.classifier3(p1)
        out = p - p1
        # out3 = torch.flatten(out, 1)
        # out1 = self.classifier(out)
        # out2 = F.normalize(self.classifier2(out), dim=1)
        return out
